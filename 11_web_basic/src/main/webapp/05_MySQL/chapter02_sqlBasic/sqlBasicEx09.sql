...
/*
    ...
    # 숫자 관련함수 
		...
		1) ROUND	: 소수점을 반올림한 값을 반환한다. (반올림할 소수점 자릿수를 지정할 수 있다.)        (암기권장)
...
/*
	...
	1) CONCAT  : 연결된 문자열을 반환한다.								(암기권장)
	2) SUBSTRING , MID : 원본문자열을 자른 문자열을 반환한다. 			(암기권장)
    3) LEFT : 원본문자열을 왼쪽부터 자른 문자열을 반환한다. 			(암기권장)
    4) RIGHT : 원본문자열을 오른쪽부터 자른 문자열을 반환한다.
    5) TRIM : 양쪽 공백을 제거한 문자열을 반환한다. 					(암기권장)
    ...
    11) LOCATE : 원본문자열에서 특정문자열의 위치(index)를 반환한다. 	(암기권장)
...
/*
	...
	# 날짜 관련 함수
		...
		1)  NOW() , SYSDATE() , CURRENT_TIMESTAMP() : 날짜와 시간관련 정보를 반환한다.    (암기권장)
		2)  CURRENT_DATE() ,  CURDATE() 			: 날짜관련 정보를 반환한다.			  (암기권장)	
