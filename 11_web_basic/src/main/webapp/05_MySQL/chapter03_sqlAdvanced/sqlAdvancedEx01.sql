...
SELECT 	 DELIVERY_STATUS ,
		 COUNT(*)
FROM	 ORDER_EX
WHERE 	 DELIVERY_STATUS <> '배송완료' # WHERE절 위치 중요
GROUP BY DELIVERY_STATUS; 	 
...
GROUP BY PRODUCT_CD
ORDER BY TOTAL_ORDER_GOODS_QTY DESC #ORDER BY절 위치 기억
LIMIT	 3;

# (GROUP BY + 함수 사용예시) 연도별로 총 주문건수 , 총 주문수량 , 평균 주문수량 조회하기
SELECT 	 SUBSTRING(ORDER_DT , 1 , 4) AS YEAR, #SUBSTRING(ORDER_DT , 1, 4)
		 COUNT(ORDER_GOODS_QTY) ,
		 SUM(ORDER_GOODS_QTY) ,
		 AVG(ORDER_GOODS_QTY)
FROM 	 ORDER_EX
GROUP BY YEAR; #SUBSTRING(ORDER_DT , 1, 4);

# (GROUP BY 2개이상 그룹화 예시) 사용자 -> 주문상품별로 그룹화하여 주문상품의 총수량을 조회하기
SELECT 	 MEMBER_ID ,
		 PRODUCT_CD ,
		 SUM(ORDER_GOODS_QTY)
FROM 	 ORDER_EX 
...
FROM     ORDER_EX 
#WHERE    SUM(ORDER_GOODS_QTY) >= 10  # 그룹바이 컬럼에 WHERE구문 사용불가
GROUP BY PRODUCT_CD 
...
SELECT   PRODUCT_CD ,
		 SUM(ORDER_GOODS_QTY) AS TOTAL_ORDER_GOODS_QTY
FROM     ORDER_EX 
WHERE 	 ORDER_DT BETWEEN '2020-01-01' AND '2020-12-31' # 기존 컬럼에 대한 조건
GROUP BY PRODUCT_CD 
HAVING   TOTAL_ORDER_GOODS_QTY >= 10;					# GROUP BY절에 대한 조건
        	
# (WHERE GROUP BY HAVING ORDER BY LIMIT 예시) 2020년에 주문된 상품 중에서 사용자 > 주문상품별로 그룹화하여 총 주문수량을 조회하되 
# 총 주문수량이 5개 이상만 조회하고 제일 많은 수량 3개의 레코드만 조회하기.
SELECT 	 MEMBER_ID ,
		 PRODUCT_CD ,
		 SUM(ORDER_GOODS_QTY) AS TOTAL_ORDER_GOODS_QTY
FROM 	 ORDER_EX 
WHERE    ORDER_DT BETWEEN '2020-01-01' AND '2020-12-31' # 2020년에
GROUP BY MEMBER_ID , # 사용자별로 그룹화하고
		 PRODUCT_CD  # 상품별로 그룹화하여 총 주문수량을 조회하되
HAVING 	 TOTAL_ORDER_GOODS_QTY >= 5 # 총 주문수량이 5개 이상만 조회하고
ORDER BY TOTAL_ORDER_GOODS_QTY DESC # 제일 많은 수량
LIMIT	 3; # 3개의 레코드만 조회하기