...
WHERE 	PRODUCT_CD IN (SELECT	PRODUCT_CD
						FROM	TB_ORDER
						WHERE	ORDER_GOODS_QTY);


# 2) 한번에 제일 많은 수량을 주문한 주문 코드 조회하기.
SELECT 	ORDER_CD
FROM	TB_ORDER 
WHERE 	PRODUCT_CD IN (SELECT	MAX(PRODUCT_CD)
						FROM	TB_ORDER);
...
SELECT	SUM(ORDER_GOODS_QTY) AS TOTAL_ORDER_CNT
FROM	TB_ORDER
WHERE 	MEMBER_ID IN (SELECT 	MEMBER_ID
						FROM	TB_MEMBER
						WHERE	MEMBER_NM = '메르켈');
...
SELECT 	SUM(ORDER_GOODS_QTY) AS TOTAL_SALES
FROM	TB_ORDER 
WHERE 	MEMBER_ID IN (SELECT 	MEMBER_ID
						FROM	TB_MEMBER
						WHERE	RESIDENCE = '서울');
