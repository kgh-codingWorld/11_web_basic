...
/*
  ...
  # ERD Diagram 생성하기
	 	...
	 	> https://www.dbdiagram.io/home 접속
	 	> create your diagram 클릭
	 	> Import > import from MySQL 클릭
	 	> DDL(Data Definition Language) 입력
...
JOIN 	TB_PRODUCT P
ON		O.PRODUCT_CD = P.PRODUCT_CD 
WHERE 	M.SEX = 'F' AND RESIDENCE = '서울';
...
ON		O.MEMBER_ID = M.MEMBER_ID 
JOIN 	TB_PRODUCT P
ON		P.PRODUCT_CD = P.PRODUCT_CD 
WHERE 	M.SEX = 'F' AND RESIDENCE IN ('서울' , '경기' , '인천');
			
			
# 4) 남자가 주문한 '상품코드별로' 그룹화하여 상품의 코드와 주문량의 합을 조회하되 주문량의 합이 적은 순서로 정렬하여 3개만 조회하기.
SELECT 	P.PRODUCT_CD AS PRODUCT_CD,
		SUM(ORDER_GOODS_QTY) AS ORDER_GOODS_QTY
FROM 	TB_ORDER O
JOIN	TB_MEMBER M
ON		O.MEMBER_ID = M.MEMBER_ID 
JOIN 	TB_PRODUCT P
...
GROUP BY PRODUCT_NM
ORDER BY TOTAL_ORDER_GOODS_QTY DESC 
LIMIT	 3;


# 6) '지역별'로 그룹화하여 배송중인 상품의 지역명 , 주문량의 총합을 조회하기
SELECT 	 M.RESIDENCE 			AS RESIDENCE, 
		 SUM(O.ORDER_GOODS_QTY) AS TOTAL_ORDER_GOODS_QTY
FROM 	 TB_ORDER O
JOIN 	 TB_MEMBER M
ON		 O.MEMBER_ID = M.MEMBER_ID 
...
JOIN	 TB_PRODUCT P
ON		 O.PRODUCT_CD = P.PRODUCT_CD 
WHERE 	 P.PRICE BETWEEN 1000000 AND 2000000
...
WHERE 	 P.PRICE BETWEEN 1000000 AND 2000000
GROUP BY PRODUCT_NM
HAVING 	 TOTAL_ORDER_GOODS_QTY >= 5
ORDER BY TOTAL_ORDER_GOODS_QTY DESC,
		 PRODUCT_NM;
...
SELECT 	SUM(P.PRICE * O.ORDER_GOODS_QTY + P.DELIVERY_PRICE) AS ANNUAL
FROM 	TB_ORDER O
JOIN	TB_PRODUCT P
ON		O.PRODUCT_CD = P.PRODUCT_CD 
AND 	O.ORDER_DT BETWEEN '2020-01-01' AND '2020-12-31';
...
JOIN	 TB_PRODUCT P
ON		 O.PRODUCT_CD = P.PRODUCT_CD
WHERE 	 O.ORDER_DT BETWEEN '2020-01-01' AND '2020-12-31'
...
SELECT 	P.PRODUCT_CD AS PRODUCT_CD ,
		SUM(P.PRICE * O.ORDER_GOODS_QTY + P.DELIVERY_PRICE) AS ANNUAL
FROM 	TB_ORDER O
JOIN	TB_PRODUCT P
ON		O.PRODUCT_CD = P.PRODUCT_CD 
GROUP BY P.PRODUCT_CD 
...
SELECT 	SUM(P.PRICE * O.ORDER_GOODS_QTY + P.DELIVERY_PRICE) AS TOTAL_PRICE
FROM 	TB_ORDER O
JOIN	TB_MEMBER M
ON		O.MEMBER_ID  = M.MEMBER_ID 
AND 	M.MEMBER_NM = '메르켈'
...
SELECT 	M.MEMBER_ID AS MEMBER_ID ,
		SUM(P.PRICE * O.ORDER_GOODS_QTY + P.DELIVERY_PRICE) AS TOTAL_ORDER
FROM	TB_ORDER O
JOIN	TB_MEMBER M
ON		O.MEMBER_ID = M.MEMBER_ID
JOIN    TB_PRODUCT P
